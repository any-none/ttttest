...
const { installConsoleSanitizer } = require('./src/logSanitizer');
...
其余每次等待的时间不得超过3秒。`;
//     const goal = `先打开这个网站：https://vip.215.im/ 然后点击“生成邮箱地址”,获取邮箱后并复制，在打开chatgpt的对话页面，然后点击创建一个账户，使用复制好的邮箱，${userData.password}作为密码，然后在显示验证码发送后在 https://vip.215.im/ 上接收自己的邮箱验证码，接下来使用${userData.fullName}作为全名，${userData.birthDate}作为出生日期（注意如果框里面要填年龄的话换算为年龄），创建账户完成后立刻导航到\`data:text/html,<html><head><title>MISSION_ACCOMPLISHED</title></head><body style=\"background:black;color:lime;display:flex;justify-content:center;align-items:center;height:100vh;font-family:monospace;\"><h1>> TASK COMPLETED SUCCESSFULLY _</h1></body></html>\`，等待15秒并结束。
...
async function runSingleRegistration() {
    ...
    try {
        ...
        const uploadResult = await uploadAuthFile(tokenData.savedFilepath, config);
        
        if (!uploadResult.uploaded) {
            console.warn('[主程序] CPA 上传最终失败，但本地 token 文件已保留');
        }

        console.log('[主程序] 本次注册流程圆满结束！');
        console.log('[主程序] Token 已保存，本地落盘和后续上传流程已执行');
...
async function checkTokenCount() {
    ...
    const files = fs.readdirSync(outputDir).filter(isActiveTokenFilename);
    return files.length;
}
